  ...
  return [
    {
      ...
      async configureServer(_server) {
        server = _server;
        if (typeof preambleCode === "undefined" && server.config.plugins.some(({ name = "none" }) => name.toLowerCase().includes("react") && !name.toLowerCase().includes("preact"))) {
          try {
      ...
      load(id) {
        ...
        if (id === contentHmrPortId)
          return contentHmrPort.replace("__CRX_HMR_TIMEOUT__", JSON.stringify(hmrTimeout)).replace("__CRX_LIVE_RELOAD__", JSON.stringify(liveReload));
      },
      closeBundle() {
        sub.unsubscribe();
          ...
          if (key === script.refId) {
            if (script.type === "module") {
              ...
              if (worldMainIds.has(script.id)) {
                ...
                const shouldUseLoader = chunk.type === "chunk" && (chunk.imports.length > 0 || chunk.dynamicImports.length > 0 || chunk.exports.length > 0);
                if (shouldUseLoader) {
                  const refId = this.emitFile({
                    type: "asset",
          ...
          if (manifest2.content_scripts)
            for (const { js = [], matches = [] } of manifest2.content_scripts)
              for (const file of js) {
                if (worldMainIds.has(prefix$1("/", file))) {
                  contentScripts.set(
                    file,
                    formatFileData({
                      type: "loader",
                      id: file,
                      refId: file,
                      fileName: getMainWorldFileName(prefix$1("/", file)),
                      matches
                    })
                  );
                } else {
...
const crx = (options) => {
  ...
  for (const p of plugins) {
    if (p && typeof p === "object" && "name" in p) {
      const orig = p.applyToEnvironment;
